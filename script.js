...
function fnum(v){return v.toLocaleString('de-DE');} function fmoney(v){return v.toLocaleString('de-DE',{style:'currency',currency:'EUR'});} function fperc(v){return (v*100).toFixed(1)+'%';}
...
function randomChange(){ return (Math.random()*0.2 - 0.1); // ±10% }

function changeHTML(change){ const arrow = change >= 0 ? '▲' : '▼'; const color = change >= 0 ? 'green' : 'red'; return <div style="font-size:0.8rem;color:${color}">${arrow} ${(change*100).toFixed(1)}%</div>; }
...
function renderTotals(filter){ const totals = calcTotals(filterCampaigns(filter)); document.getElementById('kpis-total').innerHTML =  <div class="kpi"><div class="label">Ad Spend</div><div class="value">${fmoney(totals.ad)}${changeHTML(randomChange())}</div></div> <div class="kpi"><div class="label">Impressions</div><div class="value">${fnum(totals.imps)}${changeHTML(randomChange())}</div></div> <div class="kpi"><div class="label">Clicks</div><div class="value">${fnum(totals.clicks)}${changeHTML(randomChange())}</div></div> <div class="kpi"><div class="label">Sales</div><div class="value">${fnum(totals.sales)}${changeHTML(randomChange())}</div></div> <div class="kpi"><div class="label">Revenue</div><div class="value">${fmoney(totals.revenue)}${changeHTML(randomChange())}</div></div>; }
...
document.addEventListener('DOMContentLoaded',()=>{ renderTotals('all'); renderMonthlyTable('all'); renderTrend('all'); wireFilters(); });
