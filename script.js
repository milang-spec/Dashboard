...
/* ========= Stage / Layout (scrollable by layout.js) ========= */
...
function applyLayoutConfig(){
  ...
  (cfg.order||[]).forEach(it=>{
    const el=document.getElementById(it.id); if(!el) return;
    const span=Math.max(1, Math.min(it.span||1, cfg.columns||3));
    el.style.gridColumn=`span ${span}`; board.appendChild(el);
  });
  const stage=document.getElementById('stage');
  if((cfg.mode||'fixed-scale')==='scroll'){
    document.body.style.overflow='auto';
    stage.style.position='relative'; stage.style.transform='none'; stage.style.left=stage.style.top='';
    stage.style.width='100%'; stage.style.height='auto';
    stage.style.setProperty('--chart-h','360px');
    window.removeEventListener('resize', fitStageFixed);
  }else{
    document.body.style.overflow='hidden';
    stage.style.position='absolute'; stage.style.width=BASE.w+'px'; stage.style.height=BASE.h+'px';
    fitStageFixed(); window.addEventListener('resize', fitStageFixed);
  }
}

/* ========= Daten ========= */
const D=window.DASHBOARD_DATA; const ALL_2025=D.campaigns_2025||[]; const ALL_2024=D.campaigns_2024||[];
const STATE={filter:'ALL'}; const MONTHS=['Jan','Feb','Mrz','Apr','Mai','Jun','Jul','Aug'];

/* ========= Filter ========= */
function predicateFor(f){
  switch(f){ case'ONSITE':return c=>c.site==='Onsite';
    case'OFFSITE':return c=>c.site==='Offsite';
    case'CPM':return c=>c.model==='CPM';
    case'CPC':return c=>c.model==='CPC'; default:return()=>true;}
}
const applyFilter=(list,f)=> list.filter(predicateFor(f));

/* ========= Totals ========= */
function totals(list){
  const t=list.reduce((a,c)=>({ impressions:a.impressions+(c.impressions||0),
    clicks:a.clicks+(c.clicks||0), ad:a.ad+(c.ad||0), revenue:a.revenue+(c.revenue||0),
    orders:a.orders+(c.orders||0), booking:a.booking+(c.booking||0)}),
    {impressions:0,clicks:0,ad:0,revenue:0,orders:0,booking:0});
  const ctr=safeDiv(t.clicks,t.impressions), roas=safeDiv(t.revenue,t.ad),
        cpm=safeDiv(t.ad,t.impressions/1000), cpc=safeDiv(t.ad,t.clicks),
        delivered=safeDiv(t.ad,t.booking);
...
const KPI_DEF=[
  ...
  {key:'orders',label:'Media Sales Total',fmt:fmtNum,better:'higher'},    // 5
  {key:'revenue',label:'Media Revenue Total',fmt:fmtMoney0,better:'higher'}, // 6
...
function renderKPIs(curTotals, lyTotals){
  ...
  KPI_DEF.forEach(def=>{
    const cur=curTotals[def.key]||0, ly=lyTotals[def.key]||0, d=deltaInfo(cur,ly,def.better);
    const div=document.createElement('div'); div.className='kpi';
    if(def.key==='orders'||def.key==='revenue') div.classList.add('sales-pair');
...
/* CTA unter den Sales-KPIs */
function renderSalesCTA(){
  const body=document.querySelector('#panel-kpi .kpi-body');
  const chartSec=body.querySelector('.chart-section');
  let cta=document.getElementById('salesCta');
  if(!cta){
    cta=document.createElement('div'); cta.id='salesCta'; cta.className='sales-cta';
    body.insertBefore(cta, chartSec); // direkt unter dem KPI-Grid
  }
  const link=`sales.html?filter=${encodeURIComponent(STATE.filter)}`;
...
function autoSizeChart(){
  ...
  if(cfg.mode==='scroll'){ document.getElementById('panel-kpi').style.setProperty('--chart-h','360px'); return; }
  const card=document.getElementById('panel-kpi'), header=card.querySelector('.header'),
        grid=card.querySelector('#kpiGrid'), title=card.querySelector('.chart-header');
  const used=(header?.offsetHeight||0)+(grid?.offsetHeight||0)+(title?.offsetHeight||0)+32+(12+8);
  let free=(card.clientHeight-used); free=Math.max(200, Math.min(480, free));
...
function renderTrend(list){
  ...
  list.forEach(c=>{
    ...
    const months=[]; let cur=new Date(s); cur.setDate(1);
    while(cur<=e){ const m=cur.getMonth(), y=cur.getFullYear(); if(y===2025&&m<=7) months.push(m); cur.setMonth(cur.getMonth()+1); cur.setDate(1); }
  ...
  trendChart&&trendChart.destroy(); Chart.defaults.font.size=18;
...
/* ========= Campaign Overview ========= */
function totalsPlus(list){ return totals(list); }
function renderCampaignOverview(all){
  const t=totalsPlus(all), now=new Date();
...
function renderCampaignTable(list, allList){
  ...
  [...list].sort((a,b)=>b.ad-a.ad).forEach(c=>{
  ...
  const sumF=totals(list), sumA=totals(allList);
...
function renderRerank(list){
  const tbody=document.querySelector('#rerankTable tbody'); tbody.innerHTML='';
  const rows=[...list].map(r=>({...r,clicks:r.ecpc>0?(r.ad/r.ecpc):0,revenue:r.roas*r.ad})).sort((a,b)=>b.revenue-a.revenue);
  rows.forEach(r=>{
    const tr=document.createElement('tr');
...
function bindChips(){
  ...
  bar.addEventListener('click',e=>{
    ...
    chip.classList.add('active'); STATE.filter=chip.dataset.filter; renderAll();
  });
}
...
let trendChartOnce;
function renderAll(){
  const list25=applyFilter(ALL_2025,STATE.filter), list24=applyFilter(ALL_2024,STATE.filter);
  const t25=totals(list25), t24=totals(list24);
  renderKPIs(t25,t24);
  renderSalesCTA();
  autoSizeChart(); renderTrend(list25);
...
/* ========= Boot ========= */
window.addEventListener('resize',()=>{const cfg=window.DASHBOARD_LAYOUT||{};
  if(cfg.mode!=='scroll'){fitStageFixed();autoSizeChart();trendChart?.resize();}});
document.addEventListener('DOMContentLoaded',()=>{
  applyLayoutConfig(); bindChips(); renderRerank((window.DASHBOARD_DATA||{}).rerank||[]); renderAll();
});
